...
/**
 * Utility class for form validation across the Sellora app
...
object ValidationUtils {
    ...
    // Real-time validation for TextInputLayout
    fun setupRealTimeValidation(til: TextInputLayout, validationType: ValidationType) {
        val editText = til.editText ?: return
        editText.setOnFocusChangeListener { _, hasFocus ->
            if (!hasFocus) {
                validateField(til, validationType)
            }
        }
    }
    
    fun validateField(til: TextInputLayout, validationType: ValidationType): Boolean {
        ...
        val error = when (validationType) {
            ...
            ValidationType.REQUIRED -> if (value.isEmpty()) getRequiredFieldError(til.hint?.toString() ?: "Field") else ""
        }
        
        til.error = if (error.isNotEmpty()) error else null
